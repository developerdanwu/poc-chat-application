...
export default function useClickAway({
  ...
  if (!clickable || clickable.length < 1) {
    throw Error(
      "`clickable` property must contain at least one element id. Try passing an array of element ids to useClickAway({ clickable: ['element1', 'element2'] })"
    );
  }
  ...
  if (clickable.some((e) => e.includes('#')))
    console.error(
      'Element ids in `clickable` array should not be prefixed with a `#` symbol.'
    );
